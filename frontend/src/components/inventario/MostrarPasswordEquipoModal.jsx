...
import { equipoService } from '../../services/inventarioService';

const OCULTAR_TRAS_MS = 30000;

/**
 * Revela la contraseña del equipo. El permiso (`equipos.ver_contrasenas`) ya la
 * protege; si ADEMAS la empresa activo "Confirmar contraseña para datos
 * sensibles" (Configuracion > Empresa > Configuracion de seguridad), el
 * backend responde 401 REAUTH_REQUIRED y aqui se pide confirmar (una vez cada
 * 15 minutos por sesion, no en cada revelada). Una vez revelada se oculta
 * sola a los 30s. Cada revelacion queda en auditoria (lo hace el backend).
 * Mismo patron que components/redes/MostrarPasswordModal.jsx.
 */
export default function MostrarPasswordEquipoModal({ equipo, onClose }) {
  ...
  const intentarRevelar = async () => {
    ...
    setError('');
    try {
      const res = await equipoService.revelarPassword(equipo.id);
  ...
  return (
    <Modal
      open
      onClose={onClose}
      title={`Contraseña de ${equipo.codigo_inventario}`}
